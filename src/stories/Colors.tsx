...
const CopyButton: FC<ButtonBaseProps & { copyText: string }> = ({ children, copyText, ...props }) => {
...
const Color: FC<{ color: string, fontColor?: string }> = ({ color, fontColor = 'black' }) => {
    const [hex] = useState(() => {
        const style = window.getComputedStyle(window.document.documentElement);
        return style.getPropertyValue(`--clr-${color}`);
    })

    return <ColorWrapper $color={color} $fontColor={fontColor}>
        <CopyButton
            focusRipple
        ...
        <CopyButton
            ...
            copyText={hex}
        >
            <Typography variant="overline" style={{ textTransform: 'uppercase' }} >{hex}</Typography>
        </CopyButton>
        {/*  */}
...
const Colors: FC<{ colors: string[] }> = ({ colors }) => {
    ...
    return <Grid>
        {colors.map((c, i) => <Line key={i}>
            <Typography style={{ textTransform: 'capitalize' }} variant="h6">{c.replace('-', ' ')}</Typography>
